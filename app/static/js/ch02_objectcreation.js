...
async function runExample_ch06_simpletexture() {
    ...
    const response = await fetch("http://127.0.0.1:5000/static/image/smiley.png");
...
async function runExample_ch07_text() {
    ...
    // Read character data
    const fileResponse = await fetch("http://127.0.0.1:5000/static/json/lato_data.json");
    ...
    // Create ImageBitmap from image file
    const response = await fetch("http://127.0.0.1:5000/static/image/lato.png")
    const imageBitmap = await createImageBitmap(await response.blob());
